...
{
    private static readonly double[,] chsqt = new double[2, 10]
    {
    ...
    public EntCalc(bool binmode)
    {
        ...
        this.incirc = Math.Pow(x: Math.Pow(x: 256.0,
                y: MONTEN / 2) - 1,
            y: 2.0);
    ...
    public EntCalcResult EndCalculation()
    {
        ...
        for (i = 0; i < (this.binary ? 2 : 256); i++)
        {
            if (this.prob[i] > 0.0)
            {
                this.ent += this.prob[i] * Math.Log(a: 1 / this.prob[i],
                    newBase: 2);
            }
        }
        ...
        var chiP = Math.Sqrt(d: 2.0 * this.chiSq) - Math.Sqrt(d: (2.0 * (this.binary ? 1 : 255.0)) - 1.0);
        this.a = Math.Abs(value: chiP);
        for (i = 9; i >= 0; i--)
        {
            if (chsqt[1,
                    i] < this.a)
        ...
        var compReductionPct = ((this.binary ? 1 : 8) - this.ent) / (this.binary ? 1.0 : 8.0);

        /* Return results */
        var result = default(EntCalcResult);
        ...
        result.MonteCarloErrorPct = Math.Abs(value: Math.PI - this.montePi) / Math.PI;
